...
export class BankrunConnection {
  ...
  async sendTransaction(tx: Transaction): Promise<TransactionSignature> {
    ...
    while (finalizedCount < 10) {
      const signatureStatus = (await this.getSignatureStatus(signature)).value.confirmationStatus
  ...
  async getSignatureStatus(
    ...
    return {
      ...
      value: {
        ...
        confirmationStatus: transactionStatus.confirmationStatus as TransactionConfirmationStatus,
      },
    }
  }
  ...
  async simulateTransaction(
    ...
    const returnDataNormalized = Buffer.from(simulationResult.meta?.returnData?.data).toString("base64")
