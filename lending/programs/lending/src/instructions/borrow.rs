...
pub fn process_borrow(ctx: Context<Borrow>, amount: u64) -> Result<()> {
    ...
    match ctx.accounts.mint.to_account_info().key() {
        key if key == user.usdc_address => {
            ...
            let new_accrued_value = calculate_accrued_interest(
                user.deposited_sol,
                bank.interest_rate,
                user.last_updated,
            )?;

            total_collateral = new_accrued_value * sol_price.price as u64;
        }
        _ => {
            ...
            total_collateral = new_accrued_value * usdc_price.price as u64;
        }
    }

    // Check borrowed about is lower than liquidation threshold
    let borrowable_amount = total_collateral
        .checked_mul(bank.liquidation_threshold)
        .unwrap();

    if borrowable_amount < amount {
        return Err(LendingError::BorrowThresholdReached.into());
    }
    ...
    // Update the bank and user updated states
...
pub fn calculate_accrued_interest(
    ...
    let new_value =
        (deposited_value as f64 * E.powf(interest_rate as f64 * time_diff as f64)) as u64;
