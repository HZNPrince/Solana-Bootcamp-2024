...
pub fn process_repay(ctx: Context<Repay>, amount: u64) -> Result<()> {
    ...
    if amount > user_value {
        return Err(LendingError::RepayAmountExceeded.into());
    }
    ...
    // Update that bank and user state with the updated shares
    let borrow_ratio = amount.checked_mul(bank.total_borrowed).unwrap();
